...
若公开接口只能返回充电会员试播片段、缺失完整字幕或隐藏完整播放地址，应先取得用户对使用现有登录态的明确授权。获得授权后，仍优先在登录会话中调用 B 站接口读取元数据、字幕和完整播放地址；只有接口无法提供所需信息时，才通过页面播放器进行核验或内容获取。不得读取或输出 Cookie、密码、令牌等登录凭证。
...
未经整理的自动转写只是检索索引，不是最终事实来源。股票名和专有词必须结合画面复核；画面只用于核对主播语音/字幕中已经提到的名称，不把未口播的画面股票扩展为“提及股票”。随后将其整理为仓库中的可读时间轴文字稿。

## 5. 提取并整理文字稿

使用 `sed` 分段阅读、`rg` 搜索股票名和题材词。先形成原始提取稿，再建立内部时间轴和“全部股票提及清单”，最后制作整理文字稿。此阶段不得创建或预写总结。至少标记：

- 指数、量能、情绪和赚钱效应。
- 每个题材板块的观点起止时间。
- 每一只被主播语音/字幕提到的股票、出现时间、原始语境、市场角色、强弱原因和风险。
- 次日预案、触发条件与否定条件。
- 心态、仓位和方法论表述。

整理时尽量保留作者的表达顺序、常用措辞、语气强弱、犹豫、自我修正、类比和例子，不能为了压缩篇幅改变判断强度。
...
先从语音/字幕和整理文字稿建立“主播实际说出的股票候选清单”，再用画面、上下文和必要公开信息核名。逐段扫描时不能只搜索预先知道的核心股；主播语音/字幕明确提到的股票都进入清单，包括一带而过、负面评价、对比对象、跟风股和作者不看好的股票。画面、计划表、行情软件或截图里出现但主播没有口播的股票，不进入“全部股票提及表”，也不在总结里当作作者提及股票归纳。

无法确认时保留时间点、发音线索与上下文，并标注“名称待核实”，不要凭发音强行补全或静默省略。必要时使用行情信息或公开搜索交叉核验，但不能据此改写主播观点。

本工作流的内容核实仅限股票名称。除第 2 节的视频元数据确认外，不对题材逻辑、市场数据、主播判断、新闻背景或其他术语做独立事实核验。外部行情信息只用于确认股票名称，不得扩展为额外分析。

完成股票名核实后，检查整理文字稿是否覆盖全片议题、语音/字幕明确提到的全部股票、新手教学、长篇观点和次日计划，并确认“本期全部股票提及表”完整。将文字稿写入正式路径后冻结；冻结前禁止创建或预写总结。

## 7. 生成总结

只能以冻结文字稿为唯一内容底稿生成交易日总结或对应会员笔记。总结阶段不再启动独立核实，不直接读取零散字幕、原始转写、标题、截图或记忆补写内容。若冻结文字稿发生实质修订，必须同步复查总结。
...
修改侧边栏时，必须先定位 `text: '比巴卜作手'` 的父级作者块，再在该块内定位“交易日总结”或“视频文字稿”；禁止直接匹配配置文件中第一个同名栏目，因为“小翠”等其他作者也有相同栏目名。写入后要检查本期链接只出现在“比巴卜作手”块内，不得出现在“小翠”、视野环球财经或其他作者块中。
...
构建前先用 `rg` 检查本期日期或 BVID 是否同时出现在文章、模块索引和 VitePress 侧边栏中，并确认旧日期、旧路径没有残留。随后按作者父级块检查 `docs/.vitepress/config.mts`：本期链接必须在“比巴卜作手”块内各出现一次，在其他作者块内出现零次。仅检查全文件是否存在链接不足以证明归属正确。站点构建只能验证链接页面可生成，不能替代目录收录和父级归属检查。
