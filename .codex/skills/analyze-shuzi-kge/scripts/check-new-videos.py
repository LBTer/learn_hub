#!/usr/bin/env python3
"""Check whether Shuzi Kge published videos on a given date.
...
def print_markdown(videos: list[Video], date: str) -> None:
    if not videos:
        print(f"{date} 暂未发现{UPLOADER_NAME}新视频。")
        return

    print(f"{date} 发现 {len(videos)} 个{UPLOADER_NAME}新视频：")
    for video in videos:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description=f"检查{UPLOADER_NAME}当天是否有新视频，并输出视频链接。")
