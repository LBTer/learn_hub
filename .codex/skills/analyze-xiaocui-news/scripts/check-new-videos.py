#!/usr/bin/env python3
"""Check whether Xiaocui News published videos on a given date.
...
DEFAULT_CHANNEL_URL = "https://www.youtube.com/@cui_news/streams"
...
def print_markdown(videos: list[Video], date: str) -> None:
    if not videos:
        print(f"{date} 暂未发现小翠时政财经新视频。")
        return

    print(f"{date} 发现 {len(videos)} 个小翠时政财经新视频：")
    for video in videos:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="检查小翠时政财经当天是否有新视频，并输出视频链接。")
    ...
    parser.add_argument(
        ...
        help="YouTube 频道直播页 URL，默认检查 /streams。",
    )
