#!/usr/bin/env python3
"""Check whether Yuan Shuai Ma published videos on a given date.
...
DEFAULT_SPACE_URL = "https://space.bilibili.com/401453595/video"
...
def print_markdown(videos: list[Video], date: str) -> None:
    if not videos:
        print(f"{date} 暂未发现袁帅马新视频。")
        return

    print(f"{date} 发现 {len(videos)} 个袁帅马新视频：")
    for video in videos:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="检查袁帅马当天是否有新视频，并输出视频链接。")
